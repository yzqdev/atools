...
let sidebarIntroValue = [
  {
    id: 'i', value: 'ignore - 不区分大小写', desc: `将匹配设置为不区分大小写，搜索时不区分大小写: A 和 a
                  没有区别。`
  },
  {id: 'g', value: `global - 全局匹配`, desc: `查找所有的匹配项。`},
  {
    id: 'm', value: `multi line - 多行匹配`, desc: `使边界字符 <span class="marked">^</span> 和
                <span class="marked">$</span>
                匹配每一行的开头和结尾，记住是多行，而不是整个字符串的开头和结尾。`
  },
  {
    id: 's', value: `特殊字符圆点 <span class="marked">.</span> 中包含换行符
                <span class="marked">\\n</span>`, desc: ` 默认情况下的圆点 <span class="marked">.</span> 是匹配除换行符
                <span class="marked">\\n</span> 之外的任何字符，加上
...
let state = reactive({
  regName: "", matchResult: '',
  showModal: false,
  textPattern: "", sidebarIntro: sidebarIntroValue,
  textSource: "",
  copyBtnText:'',
...
let {
  ...
  codeList, sidebarIntro,copyBtnText
} = toRefs(state);
      ...
      <el-header>
        <el-card style="margin-bottom: 20px" shadow="never">
          <el-space>
            ...
            <el-button type="info" @click.stop="setTestSource">生成测试字符串</el-button>
            <el-button
      ...
      <el-container>
        <el-main>
          <el-space fill direction="vertical" size="large" style="width: 100%">
            <el-alert type="info" v-if="containsSpace">注意,你的正则表达式内有空格,需要谨慎</el-alert>
          ...
          <el-card style="margin-top: 20px" title="常用正则表达式">
            <el-alert type="success">点击表达式可以直接在上面测试</el-alert>
            <div class="card-body">
              <h2>一、校验数字的表达式</h2>
              <ul>
                <li v-for="item in checkDigital">
                  {{ item.text }}：<strong @click="setRegTo(item)">{{
                    item.reg
                  }}</strong>
                </li>
              </ul>
        ...
        >
          <el-table :data="sidebarIntro" border fit>
            <el-table-column prop="id" label="修饰符" width="80">
            ...
            <div class="modal-body">
              ...
              <div>
                <div v-for="item in codeList">
                  ...
                  <el-button @click="clickToCopy(item)">
                    复制代码
                  </el-button>
                  {{copyBtnText}}

                    <highlightjs class="prettyprint" :code="formatStr(item.code,[textPattern,textSource])"
                                  :language="item.codeName"></highlightjs>
                </div>
              </div>
            </div>
          </div>
